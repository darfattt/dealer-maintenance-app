    ...
    <div class="p-6">
        ...
        <div class="mb-6 p-4 border border-surface-200 rounded-lg bg-surface-50">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <input v-model="testParams.dealerId" type="text" class="w-full p-2 border border-surface-300 rounded" placeholder="Enter dealer ID" />
                </div>
                <div>
                    <label class="block text-sm font-medium mb-2">Date From</label>
                    <input v-model="testParams.dateFrom" type="date" class="w-full p-2 border border-surface-300 rounded" />
                </div>
                <div>
                    <label class="block text-sm font-medium mb-2">Date To</label>
                    <input v-model="testParams.dateTo" type="date" class="w-full p-2 border border-surface-300 rounded" />
                </div>
            </div>
            <button @click="refreshWidget" class="mt-4 px-4 py-2 bg-primary text-white rounded hover:bg-primary-600">Refresh Widget</button>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <div class="h-64">
                    <PaymentRevenueWidget :key="widgetKey" :dealer-id="testParams.dealerId" :date-from="testParams.dateFrom" :date-to="testParams.dateTo" />
                </div>
            </div>
            ...
            <div>
                ...
                <div class="p-4 border border-surface-200 rounded-lg bg-surface-50 h-64 overflow-auto">
                    <button @click="testAPI" class="mb-4 px-3 py-1 bg-secondary text-white rounded text-sm hover:bg-secondary-600" :disabled="apiLoading">
                        {{ apiLoading ? 'Testing...' : 'Test API Directly' }}
                    </button>

                    <div v-if="apiError" class="text-red-600 text-sm mb-2">Error: {{ apiError }}</div>
