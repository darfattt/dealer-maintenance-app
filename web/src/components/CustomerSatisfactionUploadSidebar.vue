...
const uploadFile = async () => {
    ...
    try {
        const result = await CustomerService.uploadCustomerSatisfactionFile(selectedFile.value, overrideExisting.value, reformatDates.value);
...
<template>
    <Sidebar :visible="visible" position="right" :style="{ width: '400px' }" @update:visible="$emit('update:visible', $event)" header="Upload Customer Satisfaction" :modal="false">
        <div class="flex flex-col gap-4 h-full">
            ...
            <Card>
                ...
                <template #content>
                    <div class="space-y-4">
                        ...
                        <FileUpload mode="basic" :auto="false" accept=".xlsx,.xls,.csv" :maxFileSize="10000000" chooseLabel="Choose File" chooseIcon="pi pi-file" @select="onFileSelect" class="w-full" />
                        ...
                        <div v-if="selectedFile" class="p-3 bg-gray-50 rounded-lg border">
                            ...
                            <div class="text-xs text-gray-600 mt-1">Size: {{ formatFileSize(selectedFile.size) }}</div>
                        </div>

                        <!-- Override Option -->
                        <div class="flex items-center gap-3 p-3 bg-orange-50 border border-orange-200 rounded-lg">
                            <Checkbox v-model="overrideExisting" inputId="override-checkbox" :binary="true" />
                            <div class="flex-1">
                                <label for="override-checkbox" class="text-sm font-medium text-orange-800 cursor-pointer"> Override existing records </label>
                        ...
                        <!-- Upload Button -->
                        <Button @click="uploadFile" :disabled="!selectedFile || uploading" :loading="uploading" label="Upload File" icon="pi pi-upload" class="w-full" severity="success" />
                    </div>
                </template>
            </Card>
                ...
                <template #content>
                    <div class="space-y-2">
                        <div class="flex justify-between">
                            ...
                            <Tag :value="uploadResult.data?.upload_status || 'UNKNOWN'" :severity="getStatusSeverity(uploadResult.data?.upload_status)" />
                        </div>
                        <div class="flex justify-between">
                        ...
                        <div v-if="uploadResult.data?.override_enabled !== undefined" class="flex justify-between">
                            ...
                            <Tag :value="uploadResult.data.override_enabled ? 'Enabled' : 'Disabled'" :severity="uploadResult.data.override_enabled ? 'warning' : 'info'" class="text-xs" />
                        </div>
                    </div>
                </template>
            </Card>
            ...
            <Card v-if="showHistory">
                ...
                <template #content>
                    ...
                    <div v-else-if="uploadHistory.length === 0" class="text-center py-4 text-gray-500 text-sm">No recent uploads</div>
                    <div v-else class="space-y-2 max-h-60 overflow-y-auto">
                        <div v-for="upload in uploadHistory" :key="upload.id" class="p-3 border rounded-lg hover:bg-gray-50 transition-colors">
                            <div class="flex justify-between items-start">
                                ...
                                <Tag :value="upload.upload_status" :severity="getStatusSeverity(upload.upload_status)" class="ml-2" />
                            </div>
                            <div class="text-xs text-gray-600 mt-2">{{ upload.successful_records }}/{{ upload.total_records }} records</div>
                        </div>
                    </div>
                </template>
            </Card>
