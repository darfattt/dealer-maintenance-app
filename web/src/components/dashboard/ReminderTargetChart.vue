...
const getReminderTargetColor = (target, index) => {
    ...
    const defaultColors = ['#3B82F6', '#10B981', '#F59E0B', '#8B5CF6', '#EC4899', '#EF4444', '#6B7280', '#14B8A6'];
    ...
    <Card class="reminder-target-chart">
        ...
        <template #content>
            ...
            <div v-else-if="!hasData" class="flex flex-col items-center justify-center h-64">
                <Message severity="info" :closable="false"> No reminder target data available </Message>
            </div>
            ...
            <div v-if="hasData && !loading && sortedTargetData.length > 0" class="mt-4">
                ...
                <div class="space-y-2">
                    <div v-for="(item, index) in sortedTargetData.slice(0, 5)" :key="item.target" class="flex justify-between items-center p-2 bg-surface-50 dark:bg-surface-800 rounded-lg border border-surface-200 dark:border-surface-600">
                        <div class="flex items-center space-x-2">
                            <div class="w-3 h-3 rounded-full" :style="{ backgroundColor: getReminderTargetColor(item.target, index) }"></div>
                        ...
                        <div class="text-right">
                            ...
                            <div class="text-xs text-surface-500 dark:text-surface-400">{{ ((item.count / totalReminders) * 100).toFixed(1) }}%</div>
                        </div>
                    </div>
                </div>
                ...
                <div v-if="sortedTargetData.length > 5" class="text-center mt-2">
                    <span class="text-xs text-surface-500 dark:text-surface-400"> and {{ sortedTargetData.length - 5 }} more... </span>
                </div>
            </div>
        </template>
    </Card>
