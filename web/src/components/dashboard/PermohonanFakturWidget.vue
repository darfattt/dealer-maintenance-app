...
const error = ref('');
const permohonanFakturData = ref({
    count: 0,
    trend: 'stable',
    percentage: 0
});
...
const trendIcon = computed(() => {
    switch (permohonanFakturData.value.trend) {
        case 'up':
...
// Methods
const fetchPermohonanFakturData = async () => {
    ...
    try {
        // Try to call the document handling API for Permohonan Faktur data
        const response = await axios.get('/api/v1/dashboard/document-handling/permohonan-faktur', {
            params: {
            ...
            <div v-else class="text-center">
                ...
                <div class="mb-4">
                    <div class="text-4xl font-bold text-surface-900 mb-2">
                        {{ permohonanFakturData.count.toLocaleString('id-ID') }}
                    </div>
                </div>
