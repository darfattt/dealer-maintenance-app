...
const fetchNSCData = async () => {
    ...
    try {
        const response = await h23DashboardService.getNSCStatistics(props.dealerId, props.dateFrom, props.dateTo);
        ...
        if (response.success) {
            ...
            error.value = response.message || 'Failed to fetch NSC data';
        }
    } catch (err) {
...
<template>
    <Card class="h-full">
        <template #content>
            ...
            <!-- NSC Data -->
            <div v-if="!error && Object.keys(nscData).length > 0" class="py-4">
                <!-- Main Amount Display -->
                <div class="grid grid-cols-2 gap-4 mb-4">
                    <!-- Amount -->
                    <div class="text-center">
                        <div class="text-2xl md:text-3xl font-bold text-orange-600 dark:text-orange-400 mb-1">
                    ...
                    <!-- Total Records -->
                    <div class="text-center">
                        <div class="text-2xl md:text-3xl font-bold text-amber-600 dark:text-amber-400 mb-1">
                            {{ nscData.total_records }}
                        </div>
                        <div class="text-xs text-muted-color">Data NSC</div>
                    </div>
                </div>
            ...
            <div v-if="!loading && !error && Object.keys(nscData).length === 0" class="text-center py-8">
                ...
                <div class="inline-flex items-center justify-center w-16 h-16 bg-gray-50 dark:bg-gray-800 rounded-full mb-3">
                    <i class="pi pi-cog text-2xl text-gray-400 dark:text-gray-500"></i>
                </div>
