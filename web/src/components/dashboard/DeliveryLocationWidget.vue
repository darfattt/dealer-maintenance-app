    ...
    <Card class="h-full">
        ...
        <template #content>
            ...
            <div v-if="!error && locationData.length > 0" class="space-y-4">
                    ...
                    <div class="absolute inset-0 flex items-center justify-center">
                        <div class="relative">
                            ...
                            <svg width="200" height="120" viewBox="0 0 200 120" class="text-surface-400">
                                ...
                                <path d="M20,60 Q40,40 80,45 Q120,50 160,55 Q180,60 170,80 Q150,90 100,85 Q60,80 20,60 Z" fill="currentColor" opacity="0.3" />
                                <!-- Location dots -->
                ...
                <!-- Location Statistics -->
                <div class="space-y-3">
                    <div v-for="location in locationData" :key="location.id" class="flex items-center justify-between p-2 rounded border border-surface-200">
                        <div class="flex items-center space-x-3">
                            <div class="w-3 h-3 rounded-full" :style="{ backgroundColor: location.color }"></div>
                            <span class="text-sm font-medium text-surface-900">{{ location.name }}</span>
                        </div>
                        <div class="text-right">
                            <span class="font-bold text-sm" :style="{ color: location.color }"> {{ location.percentage }}% </span>
                        </div>
                    </div>
                </div>
            </div>
